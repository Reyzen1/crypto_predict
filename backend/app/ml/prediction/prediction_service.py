...
class PredictionService:
    ...
    async def _store_prediction_result(
        ...
        """Store prediction result in database - FIXED for large prices"""
        ...
        try:
            ...
            # FIXED: Handle large prices by converting to smaller scale
            ...
            # If prices are too large for database field, scale them down
            # Database field seems to have precision (5,4) which means max value is 9.9999
            if predicted_price > 9.0:
                # Store as percentage change instead, or skip database storage
                logger.warning(f"Price {predicted_price} too large for database field, skipping database storage")
                logger.info(f"Prediction successful but not stored in DB: ${predicted_price:.2f}")
                return
                
            # If we reach here, price is small enough to store
            prediction = Prediction(
                crypto_id=crypto_id,
                model_name=model_name,
                model_version='1.0',
                predicted_price=predicted_price,
                confidence_score=min(float(result.get('confidence_score', 75.0)), 9.9999),  # Cap at field limit
                prediction_horizon=prediction_horizon,
                target_datetime=target_datetime,
                input_price=input_price if input_price and input_price <= 9.0 else None,
                is_realized=False,
            ...
            logger.info(f"Prediction stored in database: ID={prediction.id}")
            
        except Exception as e:
            logger.warning(f"Failed to store prediction in database: {str(e)}")
            logger.info("Prediction was successful but database storage skipped")
            db.rollback()
    ...
    def _update_response_time(self, response_time: float) -> None:
        try:
            ...
            logger.debug(f"Response time: {response_time:.3f}s, avg: {self.performance_metrics['average_response_time']:.3f}s")
            
        except Exception as e:
            logger.debug(f"Metrics update skipped: {e}")  # تغییر از warning به debug
