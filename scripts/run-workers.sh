...
EOF
    ...
    if [ -d "$PID_DIR" ]; then
        ...
        # Clean any old celerybeat.pid files specifically
        if [ -f "$PID_DIR/celerybeat.pid" ]; then
            rm -f "$PID_DIR/celerybeat.pid"
            print_success "   ✅ Cleaned old celerybeat.pid file"
            fixes=$((fixes + 1))
        fi
    fi
...
get_optimized_worker_config() {
    POOL_TYPE="eventlet"  # Changed from threads to eventlet for async compatibility
    CONCURRENCY=4  # Increased for eventlet
    MAX_TASKS_PER_CHILD=1000  # Increased back for eventlet
    PREFETCH_MULTIPLIER=1  # Keep at 1
    OPTIMIZATION=""  # Remove extra optimizations
    
    print_success "🔄 Optimized settings: Eventlet pool with async compatibility"
}
...
force_kill_all_celery() {
    ...
    if [ -d "$PID_DIR" ]; then
        ...
        # Specifically clean any remaining celerybeat files
        rm -f "$PID_DIR"/celerybeat.pid 2>/dev/null || true
        rm -f "$PID_DIR"/celery_celerybeat.pid 2>/dev/null || true
    fi
...
start_all_workers() {
    ...
    local total_workers=4  # beat, data_worker, flower, ml_worker (not 5!)
    ...
    if start_service_improved "beat" "$PYTHON_CMD -m celery -A app.tasks.celery_app beat --loglevel=info --schedule=../scripts/logs/celerybeat-schedule"; then
        success_count=$((success_count + 1))
    fi
...
show_status() {
    ...
    # Expected workers (avoid duplicates)
    local expected_workers=("beat" "data_worker" "flower" "ml_worker")
    local processed_workers=()
    ...
    if [ -d "$PID_DIR" ]; then
        for pid_file in "$PID_DIR"/*.pid; do
            ...
            if [[ " ${processed_workers[@]} " =~ " ${name} " ]]; then
                continue
            fi
            
            # Skip old celerybeat references
            if [[ "$name" == "celerybeat" ]]; then
                rm -f "$pid_file"  # Clean up old file
                continue
            fi
            
            processed_workers+=("$name")
            total=$((total + 1))
...
force_kill_all_celery() {
    ...
    # Kill specific processes with improved methods
    local patterns=(
        "celery.*worker"
        "celery.*beat" 
        "celery.*flower"
    ...
    # Method 2: Direct pkill as backup
