...
erDiagram
    ...
    price_data {
        ...
        varchar(10) timeframe "NOT NULL; CHECK (timeframe IN ('1m','5m','15m','1h','4h','1d','1w','1M')); Base timeframe of the stored data"
        ...
        jsonb technical_indicators "Cached technical indicators for different timeframes.
                                    Computed asynchronously and cached here for performance.
                                    Multi-timeframe approach: one record can serve multiple timeframes.
                                    Example:
                                    {
                                        '1h': {
        ...
        timestamp candle_time "NOT NULL; Start time of the OHLC candle in UTC (must be aligned to timeframe boundaries)"
        ...
        -- 📝 NOTE: Multi-timeframe approach: applicable_timeframes calculated dynamically from candle_time and timeframe
        timestamp created_at "NOT NULL; default: Now(); Record creation timestamp"
        timestamp updated_at "NOT NULL; default: Now(); Last data update timestamp"
        ...
        CONSTRAINT chk_candle_time_aligned CHECK (
            CASE 
                WHEN timeframe = '1m' THEN 
                    EXTRACT(second FROM candle_time) = 0
                WHEN timeframe = '5m' THEN 
                    EXTRACT(second FROM candle_time) = 0 AND EXTRACT(minute FROM candle_time) % 5 = 0
                WHEN timeframe = '15m' THEN 
                    EXTRACT(second FROM candle_time) = 0 AND EXTRACT(minute FROM candle_time) % 15 = 0
                WHEN timeframe = '1h' THEN 
                    EXTRACT(second FROM candle_time) = 0 AND EXTRACT(minute FROM candle_time) = 0
                WHEN timeframe = '4h' THEN 
                    EXTRACT(second FROM candle_time) = 0 AND EXTRACT(minute FROM candle_time) = 0 AND EXTRACT(hour FROM candle_time) % 4 = 0
                WHEN timeframe = '1d' THEN 
                    EXTRACT(second FROM candle_time) = 0 AND EXTRACT(minute FROM candle_time) = 0 AND EXTRACT(hour FROM candle_time) = 0
                WHEN timeframe = '1w' THEN 
                    EXTRACT(second FROM candle_time) = 0 AND EXTRACT(minute FROM candle_time) = 0 AND EXTRACT(hour FROM candle_time) = 0 AND EXTRACT(dow FROM candle_time) = 1
                WHEN timeframe = '1M' THEN 
                    EXTRACT(second FROM candle_time) = 0 AND EXTRACT(minute FROM candle_time) = 0 AND EXTRACT(hour FROM candle_time) = 0 AND EXTRACT(day FROM candle_time) = 1
                ELSE true
            END
        )
        CONSTRAINT unique_asset_timeframe_candle UNIQUE (asset_id, timeframe, candle_time)
        
        -- 📊 Performance Indexes (optimized for multi-timeframe queries)
        INDEX idx_price_data_asset_timeframe_time (asset_id, timeframe, candle_time DESC)
        INDEX idx_price_data_candle_time_desc (candle_time DESC)
        INDEX idx_price_data_asset_candle_time (asset_id, candle_time DESC)
    ...
    price_data_archive {
        ...
        int asset_id FK "NOT NULL; Foreign key linking to assets.id"
        varchar(10) timeframe "NOT NULL; CHECK (timeframe IN ('1m','5m','15m','1h','4h','1d','1w','1M')); Base timeframe of the archived data"
        numeric(20,8) open_price "NOT NULL; Opening price for the time period in USD"
        numeric(20,8) high_price "NOT NULL; Highest price during the time period in USD"
        numeric(20,8) low_price "NOT NULL; Lowest price during the time period in USD"
        numeric(20,8) close_price "NOT NULL; Closing price for the time period in USD"
        ...
        jsonb technical_indicators "Archived technical indicators organized by timeframe"
        
        timestamp candle_time "NOT NULL; Start time of the OHLC candle in UTC (aligned to timeframe boundaries)"
        boolean is_validated "NOT NULL; default: false; Whether data has been validated"
        
        timestamp created_at "NOT NULL; default: Now(); Record creation timestamp"
        timestamp updated_at "NOT NULL; default: Now(); Last data update timestamp"
        timestamp archived_at "NOT NULL; default: Now(); When this record was moved to archive"
        ...
        CONSTRAINT chk_candle_time_aligned_archive CHECK (
            CASE 
                WHEN timeframe = '1m' THEN 
                    EXTRACT(second FROM candle_time) = 0
                WHEN timeframe = '5m' THEN 
        ...
        -- 📊 Performance Indexes (optimized for archive queries)
        ...
        INDEX idx_price_data_archive_archived_at (archived_at DESC)
        INDEX idx_price_data_archive_timeframe (timeframe)
        INDEX idx_price_data_archive_volume (volume DESC) WHERE volume > 0
        
        -- 🚀 Partitioning Strategy (PostgreSQL)
        -- PARTITION BY RANGE (candle_time) - yearly partitions for archive (longer periods)
        -- This will be implemented in migration scripts
    }
